"""
sobol.py — Sobol global sensitivity analysis on the thermal model.
...
def eval_model(row, t_array, T_ext_array, dates, profiles):
    ...
    -------
    peak_T : float
        Peak indoor temperature (°C) over the simulation window.
    pct_over : float
        % of time steps where T_in > 26 °C.
    """
...
def plot_sobol(Si, names, metric_name, filename):
    """
    Grouped bar chart of S1 and ST indices with bootstrap CI error bars.
    """
    ...
    ax.legend()
    style_axes(ax,
               title=f"Sobol sensitivity — {metric_name}",
               ylabel="Sobol index")
...
# ---------------------------------------------------------------------
# 1. Problem definition and Saltelli sampling
# ---------------------------------------------------------------------
...
# N must be a power of 2 for Saltelli; final sample count = N × (2d + 2).
N = 512
...
# ---------------------------------------------------------------------
# 2. Scenario inputs — fixed across all 7168 runs
# ---------------------------------------------------------------------
...
T_ext_array = df_week["temperature_2m"].values      # °C
dates       = df_week.index
...
# ---------------------------------------------------------------------
# 3. Main loop — one ODE solve per parameter row
# ---------------------------------------------------------------------
...
plot_sobol(Si_peak, problem["names"], "Peak T_in",       "images/sobol_peak.png")
plot_sobol(Si_pct,  problem["names"], "% hours > 26 °C", "images/sobol_pct.png")